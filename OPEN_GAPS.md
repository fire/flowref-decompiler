...
### Class B — instructions outside the faithful class
...
**Scalar `div`/`idiv`** (`gcd`, `count_divisors`, `is_prime`, `lcm`): the
...
**64-bit magic-constant division** (`digit_count` was solved; `pow_uint`,
`is_prime` use 64-bit `imul`): these require a 64-bit Word path or
recognition of the `imul r64, r64; shr $k, r64` compound pattern.

**Unresolvable phi** (`ctz`, `russian_mul`): `ctz`'s loop-carried phi is
...
Both are Track B (readability). Do after Class B is exhausted.
