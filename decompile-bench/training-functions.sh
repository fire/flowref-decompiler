...
#
# NOTE: Gap fixtures (OPEN_GAPS.md) are tested separately via gap-bench.sh
# and are NOT included here until they achieve EQUIVALENT status.
TRAINING_FUNCS="id32 add2 umax umin abs_diff gray_code avg_floor \
       ...
       to_sbyte to_shalf shift_r shift_l rotate_right ctz_nonzero bsr_nonzero lzcnt_nonzero bswap32 add_high_carry mul_high_u32 mul_low_u32 mul_imm div_by_10 addr_calc scale8 nand \
       sel_nz is_zero cmp_lt cmp_le cmp_eq nonzero is_even in_range div_guarded branch_select branch_select_slt branch_phi_add branch_phi_twouse nested_select_cfg \
