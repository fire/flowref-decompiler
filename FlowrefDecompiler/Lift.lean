...
/-- Check if an instruction is a division and extract the divisor operand. -/
def isDivAndDivisor (i : Ins) : Option String :=
  let ops := (i.ops.splitOn ",").map (·.trimAscii.toString)
  if i.mn == "div" ∨ i.mn == "idiv" then
    match ops with
    | [_, s] => some (canonReg s)  -- divisor is the second operand
    | _ => none
  else
    none

/-- Add a path fact from a test instruction (e.g., `test r,r` followed by `jne` means r != 0). -/
def addTestFact (l : PathFactLattice) (reg : String) (isNonzero : Bool) : PathFactLattice :=
  if isNonzero then l.add (.nonzero reg) else l.add (.zero reg)
...
`cmovcc` can be fused. Also tracks path facts (for Gap 1: proving divisor != 0).
Refuses (`none`) on an unmodelled instruction or a `cmovcc` with no preceding `cmp`. -/
def fuse (argRegs : List String) (cmp : Option (String × String)) (facts : PathFactLattice) : List Ins → Option (List SInsn)
  | []        => some []
  | i :: rest =>
    if i.mn == "cmp" then
...
forwards `argRegs` to the callee. Tracks path facts for Gap 1 (proving divisor != 0). -/
def liftFn (argRegs : List String) (is : List Ins) : Option SProg :=
  (fuse argRegs none PathFactLattice.bot is).map (liftS argRegs)
